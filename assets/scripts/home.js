...
`;
  ...
  data.data.tours.forEach((tour) => {
    ...
    tempTour = tempTour.replace(
      "{@tourUrl@}",
      `https://tn-travelxd.herokuapp.com/tour.html?id=${tour._id}`
    );
    tempTour = tempTour.replace(
      "{@tourUrl@}",
      `https://tn-travelxd.herokuapp.com/tour.html?id=${tour._id}`
...
const getTour = async () => {
  const data = await ftechAPI(
    "https://tn-travelxd.herokuapp.com/api/v1/tours?limit=16&page=1&sort=-recommend"
  );
...
const getLocations = async () => {
  const data = await ftechAPI(
    "https://tn-travelxd.herokuapp.com/api/v1/locations?fields=name"
  );
...
const find = async () => {
  ...
  if (findtxb.value === "") {
    const data = await ftechAPI(
      `https://tn-travelxd.herokuapp.com/api/v1/tours/find/${
    ...
    const data = await ftechAPI(
      `https://tn-travelxd.herokuapp.com/api/v1/tours/find?text=${
