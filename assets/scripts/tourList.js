const getTours = async () => {
  const fetchRawTour = await ftechAPI(
    "https://tn-travelxd.herokuapp.com/api/v1/tours/raw",
    "GET",
    JSON.stringify({})
  );
...
const deleteTour = async (id) => {
  ...
  const fetchdelete = await ftechAPI(
    "https://tn-travelxd.herokuapp.com/api/v1/tours/" + id,
    "DELETE",
    JSON.stringify({})
  );
...
const showTours = async (tours) => {
  ...
  tours.data.tours.forEach((tour) => {
    ...
    tempTour = tempTour.replace(
      "{@approval@}",
      "https://tn-travelxd.herokuapp.com/view-don-hang/QLdonhang.html?id=" +
        tour._id
    );
    tempTour = tempTour.replace("{@status@}", tour.status);
    tempTour = tempTour.replace(
      "{@edit@}",
      "https://tn-travelxd.herokuapp.com/view-tour/sua-tour.html?id=" + tour._id
    );
    tempTour = tempTour.replace(
      "{@startAt@}",
...
const findTour = async () => {
  const data = await ftechAPI(
    `https://tn-travelxd.herokuapp.com/api/v1/tours/raw/find?text=${
